...
**Status (2026-09-25):** canon sync against the 2026-09 keystone CV — `llms.txt` and the profile README now state "367 public tests" directly; the "CV summarizes it as 270 unit tests" bridge is dropped (the keystone CV says 367). command-code-mods corrected to **nine** mods: the suite's own `mods/index.md`, CHANGELOG `[Unreleased]`, and contract check always tracked nine (protocol-loader + error-tracker shipped under `[Unreleased]` without a README update); only the README — and the surfaces that had copied it — said seven. Repo README/package.json, `llms.txt`, the project page, the hub keystone, and the keystone CV (claims register row; banned: cmd-mods, six, seven) all now say nine. Dated entries below keep prior wording as history.

**Status (2026-09-21):** portfolio polish pass for the six pinned repos — lead walkthrough frames now perform each repo thesis (WCP rejected audit drawer, aria blocked injection, eval failed case, agenttrace replay, gateway simulate panel); all 17 figures carry narrative captions plus **Walkthrough** headings; catalog/home thumbnails match new leads; six READMEs use numbered click paths. Prior walkthrough infrastructure: sample-data PNGs from forced demos, thesis lines on cards, `styles.css` walkthrough utilities. Hub decision: `docs/decisions/2026-09-21-portfolio-demo-walkthroughs.md`.

**Status (2026-09-14):** pinned spotlight realigned to `FishRaposo/FishRaposo` profile README (2026-09-13): WCP V5, groundtruth, aria-agent, evalforge, agenttrace, llm-gateway in pin order; `command-code-mods` demoted to "Also ship" (shipped, not pinned); `llm-gateway` promoted from supporting-only. Updated `projects.html`, `index.html`, `llms.txt`, project-page badges, and sitemap lastmods. Copy polish: `index.html` `og:description`/keywords and `projects.html` CTA subtitle reordered to profile pin order; `llms.txt` public repo count 32 → 33 (GitHub API) with inventory note for hermes-agent and portfolio-builder (public archived, no project page yet).

**Status (2026-09-13):** entrega-clara added to the catalog (project page, `projects.html` card, `llms.txt` entry, sitemap; public repo count 31 → 32). Same day, per the owner's correction, the Expat Money metric was re-scoped on `index.html` and `llms.txt`: the 90% is the **content-production AI tooling bill** (a $200/month subscription replaced by a custom GPT on ChatGPT Plus) — not overall content costs, which were never measured; "production time halved" is unchanged. The dated entries below keep the old wording as history.
...
**Status (2026-08-23, fixed-pin rename reconciliation — superseded 2026-09-14):** The public `cmd-mods` repository was renamed to `FishRaposo/command-code-mods`; the catalog, manifest, sitemap, and project page use that canonical name. That pass briefly listed command-code-mods as a fixed pin and demoted llm-gateway to supporting-only; the 2026-09-14 realignment restores the profile README six (WCP V5, groundtruth, aria-agent, evalforge, agenttrace, llm-gateway) and labels command-code-mods as "Also ship". The NDS toolkit is documented in the hub as a future spotlight switch once its public-safe benchmark receipt, ROM-free demo, CI, and proof boundaries are ready.
